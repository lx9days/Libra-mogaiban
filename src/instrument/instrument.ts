...
export default class Instrument {
  ...
  async _dispatch(layer: Layer<any>, event: string, e: Event) {
    ...
    if (event === "mousemove" && e instanceof MouseEvent) {
      ...
      const hardConflictLayersByInst = new Map<Instrument, Set<string>>();
      const softConflictLayersByInst = new Map<Instrument, Set<string>>();

      const getInstrumentLayers = (inst: Instrument): Layer<any>[] => {
        const layers = new Set<Layer<any>>();
      ...
      const conflictInstsByLayer = new Map<Layer<any>, Instrument[]>();
      ...
      conflictInstsByLayer.forEach((insts, layr) => {
        ...
        const hasUncontrolledInst = insts.some(
          (inst) => !hasConflictControl(inst)
        );
        if (!hasUncontrolledInst) return;
      ...
      const buildHudInstrumentHtml = (inst: Instrument) => {
        ...
        if (trigger) html += ` <span style="color: #ffd54f;">{${trigger}}</span>`;
        if (hasHardConflict) {
        ...
        if (synthetic) html += ` <span style="color: #4db6ac;">[${synthetic}]</span>`;
    ...
    for (let [inter, layr, layerOption, instrument] of layers) {
      ...
      if (e instanceof MouseEvent) {
        if (
          ...
          pickingResult = [];
          try {
            const rect = (layr.getGraphic() as any).getBoundingClientRect?.();
            if (rect) {
